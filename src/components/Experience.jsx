  ...
  return (
    <Section id="experience" phase={5} label={exp.label}>
      <div className="relative pl-[clamp(28px,4vw,44px)]">
        ...
        <ol className="list-none m-0 p-0 flex flex-col gap-16">
          {exp.items.map((item) => (
            <Reveal as="li" key={item.period + item.title} className="relative">
              <span
                className="absolute top-[7px] w-2.5 h-2.5 rounded-full bg-current"
                style={{ left: "calc(8px - clamp(28px, 4vw, 44px) - 4.5px)" }}
                aria-hidden="true"
              />
              <p className="m-0 mb-1.5 text-[13px] font-semibold tracking-[0.18em] uppercase opacity-70">
              ...
              {item.highlights.length > 0 && (
                <ul className="list-none m-0 mb-4 p-0 flex flex-col gap-2 max-w-[68ch]">
                  {item.highlights.map((h, i) => (
                    <li key={i} className="leading-[1.6] opacity-[0.78] text-[15.5px] pl-5 relative">
                      <span className="absolute left-0 top-[0.68em] w-[11px] h-[1.5px] bg-current" aria-hidden="true" />
                      {h}
                    </li>
                  ))}
                </ul>
